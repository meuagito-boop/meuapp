...
| `frontend/src/screens/main/SettingsAuxScreens.tsx` | Bloqueados | Gerenciar bloqueios | Nao implementado | Lista vazia fixa sem backend de bloqueios | `SettingsAuxScreens.tsx` | Implementar lista real ou ocultar entrada de Privacidade |
...
| P1 | `frontend/src/screens/main/SettingsAuxScreens.tsx:102-154`, `188-210` | Contas vinculadas, raio, preferencias de notificacao, idioma e bloqueados continuam linhas estaticas/switches disabled; alterar senha foi RESOLVIDO pela EXECUCAO-011 | Nao para telas de configuracao acionaveis | Estado real das preferencias, provedores vinculados e bloqueios | Notificacoes: `notificationsService`; demais exigem endpoints de preferencias/bloqueios | Usuario altera nada apesar da UI parecer area de conta real; suporte e privacidade ficam inconsistentes |
...
5. RESOLVIDO parcialmente no codigo local pela EXECUCAO-011: alterar senha foi implementado; dispositivos/historico foram retirados do menu de seguranca e nao exibem dados inventados. Ainda falta ocultar ou implementar privacidade, notificacoes, idioma, bloqueados e raio sem contrato real.
...
| `frontend/src/screens/main/SettingsAuxScreens.tsx` | Notificacoes, idioma, bloqueados, alterar senha, suporte/legal auxiliares | Ajustar preferencias e acessar suporte/legal | Alterar senha sim; demais parcial/nao comprovado | Parcial/nao comprovado | Parcial | Classificar item a item; alterar senha RESOLVIDO pela EXECUCAO-011; conectar ou ocultar demais | Remover fallback local, conectar preferencias reais, suporte real e legal real | P1; P0 para itens visiveis fake |
